...
#[derive(Debug, Serialize, Clone, Builder, PartialEq, Eq)]
#[serde(rename = "object")]
pub struct Object {
...
pub fn parse_neww_ui(neww_ui: &str) -> Result<Option<Object>, anyhow::Error> {
    let mut reader = Reader::from_str(neww_ui);
    ...
    loop {
        ...
        match event {
            quick_xml::events::Event::Start(tag) => {
                root = Some(parse_object(tag, &mut reader)?);
            }
            quick_xml::events::Event::End(_tag) => return Err(anyhow!("unexpected end tag")),
            quick_xml::events::Event::Empty(tag) => {
                root = Some(parse_object_empty_tag(tag, &mut reader)?);
            }
            quick_xml::events::Event::Text(_txt) => {}
            quick_xml::events::Event::Decl(_) => continue,
